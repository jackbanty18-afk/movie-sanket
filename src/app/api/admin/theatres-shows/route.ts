...
import { getRolesByEmail, listTheatres, upsertTheatre, deleteTheatre, listShows, upsertShow, deleteShow, listMovies, type TheatreRow, type ShowRow } from "@/lib/db";
...
export async function GET(req: NextRequest) {
  ...
  if (kind === "shows") {
    ...
    return Response.json({ shows: listShows({ movieId: movieId || undefined, dateKey: dateKey || undefined, publishedOnly: false }) });
  }
  if (kind === "movies") {
...
export async function POST(req: NextRequest) {
  ...
  if (kind === "theatres") {
    ...
    upsertTheatre(row as TheatreRow);
...
export async function DELETE(req: NextRequest) {
  ...
  if (kind === "theatres") { deleteTheatre(id); return Response.json({ ok: true }); }
  if (kind === "shows") { deleteShow(id); return Response.json({ ok: true }); }
  return new Response(JSON.stringify({ error: "unknown kind" }), { status: 400 });
}
