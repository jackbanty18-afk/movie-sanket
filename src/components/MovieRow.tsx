...
export default function MovieRow({ title, movies }: { title: string; movies: MovieCardItem[] }) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      {movies.length === 0 ? (
        <p className="mt-6 text-white/70">No items.</p>
      ) : (
            ...
            {movies.map((m) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 translate-y-6 opacity-0 p-4 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                  <h4 className="text-lg font-extrabold text-white drop-shadow-md">{m.title}</h4>
